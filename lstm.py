...
### RUN COMMAND ###
# python3 lstm.py 250 20 mse random_uniform 0.0001 30 adam 0.4
### RUN COMMAND ###
...
def create_model():
    ...
    lstm_model.add(
        LSTM(34, batch_input_shape=(BATCH_SIZE, TIME_STEPS, x_t.shape[2]), dropout=0.0, recurrent_dropout=0.0,
             stateful=True, kernel_initializer=kernel, activation='tanh', return_sequences=True))

    lstm_model.add(Dropout(0.2))
    lstm_model.add(LSTM(34, return_sequences=True, activation='tanh'))
    lstm_model.add(Dropout(0.2))
    lstm_model.add(LSTM(34, activation='tanh'))
    lstm_model.add(Dropout(0.2))
    lstm_model.add(Dense(1))
    print("number of layers: ", len(lstm_model.get_config()['layers']))
    lstm_model.compile(optimizer=optimizer, loss=lossf_v, metrics=['mse', 'mae', 'msle', 'logcosh'])

    # change this path accordingly
    csv_logger = CSVLogger(
        os.path.join(OUTPUT_PATH, 'lstm1' + '.log'),
        append=True)
    ...
    lstm_model_name = foldername + "/modelnew_impt5.h5"

    if not os.path.exists('Models/'):
        os.mkdir('Models/')

    if os.path.exists(lstm_model_name):
        os.remove(lstm_model_name)
    lstm_model.save(lstm_model_name)

    print(lstm_model.get_config())
    return lstm_model, history


if __name__ == '__main__':

    foldername = os.path.join(OUTPUT_PATH, "model_lstm_ep_" + str(sys.argv[1]) + "_bs_" + str(sys.argv[2]) + "_loss_" + str(
